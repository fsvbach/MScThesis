...
It it easy to see that the Gaussian approximation in Figure \ref{EVSoverview} is far from accurate, however we argue that it is still more accurate than taking only the mean. Also we will find in section \ref{WTevs} that the approximation leads to fair results compared to the Exact Wasserstein embedding. Later, in section \ref{GER} we will analyse a continuous dataset that perfectly fits the Gaussian approxmiation.
