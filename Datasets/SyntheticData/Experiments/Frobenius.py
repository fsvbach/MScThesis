...
def run(seed=None, n=20, output=False, suffix=''):
    ...
    mixture = HGM(seed=seed, classes=4, datapoints=n)
    
    timer.add(f'create {n} Gaussians')
    
    FrobeniusDistance = GWD(mixture.data, fast_approx=True)
    timer.add('...calculated Frobenius Distance')
    
    ExactWasserstein  = GWD(mixture.data)
    ...
    A = FrobeniusDistance.Gaussians[a].cov
    B = ExactWasserstein.Gaussians[b].cov
